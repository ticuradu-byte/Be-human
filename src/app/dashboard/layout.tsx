...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const Sidebar = () => (
    <aside className="w-64 flex-shrink-0 h-screen flex flex-col bg-[#0f1419]/80 overflow-hidden border-r border-white/[0.06]">
      {/* Logo */}
      <div className="p-5 border-b border-white/[0.06]">
        <Link href="/dashboard" className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[9px] text-white/30 uppercase tracking-[1.5px] mt-0.5">Wellness AI</div>
          </div>
        </Link>
      </div>
      ...
      {trialActiv && (
        <div className="mx-3 mt-3 px-3 py-2 bg-green-500/8 border border-green-500/20 rounded-xl">
          ...
          <div className="text-[10px] text-white/30 mt-0.5">{trialZile} zile rămase</div>
        </div>
      )}
        ...
        {NAV.map(item => (
          <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
            ...
            {item.href === '/dashboard/wearables' && !areAcces(util?.plan || 'free', 'wearables_api') && (
              <span className="ml-auto text-[9px] text-white/30 uppercase">Plus</span>
            )}
          </Link>
        ))}
      </nav>
      ...
      <div className="p-4 border-t border-white/[0.06]">
        ...
        {analizeRamase !== null && (
          <div className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-3 mb-3">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[10px] text-white/30">{util?.analize_luna || 0}/{planInfo.analize}/lună</span>
            </div>
            ...
            <div className="text-[10px] text-white/30 mt-1">{analizeRamase} analize rămase</div>
        ...
        {analizeRamase === null && (
          <div className="bg-green-500/[0.05] border border-green-500/[0.15] rounded-xl p-3 mb-3">
            <div className="flex items-center gap-2">
              <span className={planBadgeClass[util?.plan || 'free']}>{planInfo.label}</span>
              <span className="text-[10px] text-white/30">Analize nelimitate</span>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-white/30 text-[10px] truncate">{util?.email}</div>
          </div>
          <button onClick={logout} className="text-white/20 hover:text-white/45 transition-colors text-xs" title="Logout">⏏</button>
        </div>
      </div>
    </aside>
  )
  ...
  return (
    <div className="min-h-screen bg-[#0f1419] flex">
      ...
      <main className="flex-1 flex flex-col min-h-screen">
        {/* Mobile header */}
        <div className="md:hidden flex items-center justify-between px-4 py-3 border-b border-white/[0.06] bg-[#0f1419]/80">
          <button onClick={() => setMobileOpen(true)} className="text-white/45 text-xl">☰</button>
